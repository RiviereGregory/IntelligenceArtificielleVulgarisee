...
poids = {
    ...
    'couche_entree_vers_cachee': tf.Variable(tf.random_normal([60, 26]), tf.float32),

    #26 neurones de la couche cachée vers 2 de la couche de sortie
    'couche_cachee_vers_sortie' : tf.Variable(tf.random_normal([26, 2]), tf.float32),
}

poids_biais = {
     #1 biais de la couche d'entrée vers les 24 neurones de la couche cachée
    'poids_biais_couche_entree_vers_cachee': tf.Variable(tf.zeros([26]), tf.float32),
